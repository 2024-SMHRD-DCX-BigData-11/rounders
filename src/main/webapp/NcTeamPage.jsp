...
<body>
<%List<TeamBoardDTO> showNcBoard = new BoardDAO().showNcBoard();
MatchDTO mymatch = new MatchDAO().myView(6);
...
if(category.equals("전체")){
	...
	showNcBoard = new BoardDAO().showKiaBoard();
}else if(category.equals("잡담")){
	category = "잡담";
	System.out.println(category);
	TeamBoardDTO myboard= new TeamBoardDTO(category,6);
	showNcBoard = new BoardDAO().selectTeamBoardCategory(myboard);
}
...
<div class = "wrap">
		...
		<div  id="team">
			<h1>
				NC 다이노스 게시판 <select name = "board_category" onchange="if(this.value) location.href=(this.value);">
							...
							<option value="NcTeamPage.jsp?category=직관모임">직관모임</option>
				</select>
			</h1>
			<table class="table">
			<span style="color: red">!! 지금 핫한 게시물 !! >>></span>
				...
				<%for(TeamBoardDTO tb : showNcBoard){%>
	...
	<div class = "write_wrap">
		...
		<%}else if(mem_login != null && team_idx == 6){%>
