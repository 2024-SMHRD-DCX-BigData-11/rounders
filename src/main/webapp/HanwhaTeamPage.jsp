...
<body>
<%List<TeamBoardDTO> showHanwhaBoard = new BoardDAO().showHanwhaBoard();
 MatchDTO mymatch = new MatchDAO().myView(7);
 ...
 if(category.equals("전체")){
 	...
 	showHanwhaBoard = new BoardDAO().showKiaBoard();
 }else if(category.equals("잡담")){
 	category = "잡담";
 	System.out.println(category);
 	TeamBoardDTO myboard= new TeamBoardDTO(category,7);
 	showHanwhaBoard = new BoardDAO().selectTeamBoardCategory(myboard);
 }
...
<div class = "wrap">
	<div class="all">
		...
		<div  id="team">
			<h1>
				한화 이글스 게시판 <select name = "board_category" onchange="if(this.value) location.href=(this.value);">
							<option value="카테고리">카테고리</option>
							<option value="HanwhaTeamPage.jsp?category=전체">전체</option>
							<option value="HanwhaTeamPage.jsp?category=잡담">잡담</option>
							<option value="HanwhaTeamPage.jsp?category=정보">정보</option>
							<option value="HanwhaTeamPage.jsp?category=직관모임">직관모임</option>
				</select>
			</h1>
			<table class="table">
			<span style="color: red">!! 지금 핫한 게시물 !! >>></span>
				...
				<%for(TeamBoardDTO tb : showHanwhaBoard){%>
	...
	<div class = "write_wrap">
		...
		<%}else if(mem_login != null && team_idx == 7){%>
