...
<html>
	...
	<%
		...
		if(login_member != null){
			list = new MessageDAO().selectMessage(login_member.getEmail());
					...
					<header id="header" class="alt">
						...
						<nav>
							...
							<%if(login_member == null){ %>
								<a href="#menu">로그인</a>
							<%}else{ %>
								<%if(login_member.getEmail().equals("admin")){ %>
						...
						<!-- Q1. 회원가입 기능 만들기 -->
							<li><h5>회원가입</h5></li>
								<form action="JoinCon.do" method="post">
									<li><input type="text" id="inputEmail" placeholder="Email을 입력하세요" name="email"></li>
									
									<li><input type="button" id="inputE" value="중복체크" onclick="checkE()"></li>
				...
				<!-- Banner -->
					<section id="banner" class="major">
						<div class="inner">
							...
							<%}else{ %>
								<h1><%=login_member.getEmail() %>님 환영합니다~!</h1>
							<%} %>
							</header>
							<div class="content">
								<p>게시판을 이용해보세요 ^^<br></p>
							...
							<section id="two">
								<div class="inner">
									...
									<ul class="actions">
										...
										<!-- Q14. 메시지 전체 삭제 기능 -->
										<% if(login_member != null) {%>)								
										<li><a href="MessageRemoveCon.do?email=<%=login_member.getEmail() %>" class="button next scrolly">전체삭제하기</a></li>
										<%}else{ %>
							...
							<!-- 다른 사람의 DB에 메시지 저장해보기! -->
								...
								<div class="field half first">
									<%if(login_member != null){ %>
										...
										<input type="text" name="sender" value="<%=login_member.getEmail() %>" id="name" placeholder="보내는 사람 이름" />
									<%}else{ %>
										<label for="name">Name</label>
							...
							<section class="split">
								<section>
									<div class="contact-method">
										...
										<%}else{ %>
											<a href="#"><%=login_member.getEmail() %></a>
										<%} %>
									</div>
								</section>
								<section>
								...
								<section>
									<div class="contact-method">
										...
										<%}else{ %>
											<span><%=login_member.getAddress() %></span>
										<%} %>
									</div>
								</section>
							</section>					
			...
			function checkE(){
				...
				$.ajax({ // { key1 : value1, key2 : value2, key3 : {}}
					...
					success : function(data){
						/* alert(data); */
						if(data=='true'){
							$('#resultCheck').text('사용할 수 없는 이메일');
							$('#resultCheck').css('color', 'red');
						}else if(data=='false'){
							$('#resultCheck').text('사용할 수 있는 이메일');
						}
					},
					error : function(){
						alert('error');
